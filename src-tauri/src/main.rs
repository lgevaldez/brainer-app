...
use agents_contract::render_master_root_agents_template;
use workspaces::{discover_workspace_children_entries, FolderEntry};
...
fn discover_workspace_children(root_path: String) -> Result<Vec<FolderEntry>, String> {
    ...
    discover_workspace_children_entries(path)
}
...
#[tauri::command]
fn get_cognitive_status() -> Result<CognitiveStatusPayload, String> {
    let client = Client::builder()
        .timeout(Duration::from_secs(10))
        .build()
        .map_err(|err| format!("Failed to build HTTP client: {}", err))?;
    ...
    let telemetry = client
        .get(format!("{}/api/memory/telemetry", BACKEND_URL))
        .send()
        .map_err(|err| format!("Failed to call memory telemetry endpoint: {}", err))?
        .json::<serde_json::Value>()
        .map_err(|err| format!("Invalid JSON in memory telemetry response: {}", err))?;

    let background = client
        .get(format!("{}/api/memory/background", BACKEND_URL))
        .send()
        .map_err(|err| format!("Failed to call background cognition endpoint: {}", err))?
        .json::<serde_json::Value>()
        .map_err(|err| format!("Invalid JSON in background cognition response: {}", err))?;

    Ok(CognitiveStatusPayload { telemetry, background })
}
